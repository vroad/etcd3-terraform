...
resource "aws_s3_bucket_object" "server_crt" {
  count   = "${var.cluster_size}"
  bucket  = "${aws_s3_bucket.files.id}"
  key     = "certs/${local.hosts[count.index]}/server.crt"
  content = "${tls_locally_signed_cert.server[count.index].cert_pem}"
  etag    = "${md5(tls_locally_signed_cert.server[count.index].cert_pem)}"
}

resource "aws_s3_bucket_object" "server_key" {
  count   = "${var.cluster_size}"
  bucket  = "${aws_s3_bucket.files.id}"
  key     = "certs/${local.hosts[count.index]}/server.key"
  content = "${tls_private_key.server[count.index].private_key_pem}"
  etag    = "${md5(tls_private_key.server[count.index].private_key_pem)}"
}
